...
public enum GeckoStorageController {
    ...
    /// Distinct hosts with cookies or quota-managed storage (IndexedDB,
    /// Cache API, localStorage, service workers, etc.).
    public static func listStorageHosts() async throws -> [String] {
        let response = try await GeckoEventDispatcherWrapper.runtimeInstance.query(
            type: "GeckoView:ListStorageHosts"
        )
        guard let dictionary = response as? [String: Any],
              let hosts = dictionary["hosts"] as? [String] else {
            return []
        }
        return hosts
    }
