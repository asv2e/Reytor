...
final class SiteSettingsStore {
    ...
    private enum SettingColumn: String {
        ...
        case javascriptBlocked = "javascript_blocked"
    }
    ...
    func settingsWithJavaScriptBlocked() -> [SiteSettingsRecord] {
        return stateQueue.sync {
            settingsLocked(where: "javascript_blocked IS NOT NULL")
        }
    }
    ...
    // NoScript-style per-site override. `nil` (the default, no row) means
    // "use the global JavaScript default"; an explicit true/false always
    // wins over that default for this host.
    func javascriptBlocked(for url: URL) -> Bool? {
        guard let host = URLUtils.normalizedHost(url.host) else {
            return nil
        }
        
        return stateQueue.sync {
            settingsLocked(for: host)?.javascriptBlocked
        }
    }
    
    @discardableResult
    func setJavaScriptBlocked(_ blocked: Bool?, forHost host: String) -> Bool {
        guard let host = URLUtils.normalizedHost(host) else {
            return false
        }
        ...
        return stateQueue.sync {
            guard let blocked else {
                return clearSettingLocked(.javascriptBlocked, for: host)
            }
            
            return setIntSettingLocked(blocked ? 1 : 0, column: .javascriptBlocked, for: host)
        }
    }
    
    @discardableResult
    func setJavaScriptBlocked(_ blocked: Bool?, for url: URL) -> Bool {
        guard let host = URLUtils.normalizedHost(url.host) else {
            return false
        }
        
        return setJavaScriptBlocked(blocked, forHost: host)
    }
    
    func clearAllJavaScriptBlockedSettings() -> Bool {
        return stateQueue.sync {
            clearAllSettingsLocked(column: .javascriptBlocked)
        }
    }
    ...
    private func createSchemaLocked() {
        ...
        CREATE TABLE IF NOT EXISTS site_settings (
            ...
            javascript_blocked INTEGER NULL CHECK(javascript_blocked IS NULL OR javascript_blocked IN (0, 1)),
        ...
        CREATE INDEX IF NOT EXISTS idx_site_settings_javascript_blocked ON site_settings(javascript_blocked) WHERE javascript_blocked IS NOT NULL;
        """
        
        _ = executeLocked(sql)
        
        // Existing installs created the table before this column existed;
        // CREATE TABLE IF NOT EXISTS won't add it to them.
        ensureColumnLocked(name: "javascript_blocked", table: "site_settings", definition: "INTEGER NULL")
    }
    ...
    private func settingsLocked(for host: String) -> SiteSettingsRecord? {
        guard let statement = prepareStatementLocked(
            """
            SELECT host, page_zoom, website_mode, reader_mode, javascript_blocked, created_at, updated_at
            FROM site_settings
            WHERE host = ?
            LIMIT 1;
            """
    ...
    private func deleteEmptySettingsLocked(for host: String) -> Bool {
        guard let statement = prepareStatementLocked(
            ...
            WHERE host = ?
                ...
                AND javascript_blocked IS NULL;
            """
        ) else {
            return false
        }
    ...
    private func record(from statement: OpaquePointer) -> SiteSettingsRecord? {
        ...
        if sqlite3_column_type(statement, 4) == SQLITE_NULL {
            javascriptBlocked = nil
        } else {
            javascriptBlocked = sqlite3_column_int(statement, 4) != 0
        }
