...
final class SiteSettingsViewController: UITableViewController, UINavigationControllerDelegate {
    ...
    @objc private func javaScriptSwitchDidChange(_ sender: UISwitch) {
        let blocked = !sender.isOn
        if blocked == Prefs.JavaScriptPreferences.blocksByDefault {
            // Matches the global default again - drop the per-site row
            // instead of keeping a redundant override around.
            _ = SiteSettingsStore.shared.setJavaScriptBlocked(nil, forHost: host)
        } else {
            _ = SiteSettingsStore.shared.setJavaScriptBlocked(blocked, forHost: host)
        }
        session.updateSettings(
            GeckoSessionSettings(
                websiteMode: session.settings.websiteMode,
                pageZoom: session.settings.pageZoom,
                language: session.settings.language,
                allowJavascript: sender.isOn
            )
        )
        session.reload()
    }
    ...
    private func updateSessionPageZoom(_ level: Int) {
        session.updateSettings(
            GeckoSessionSettings(
                ...
                language: session.settings.language,
                allowJavascript: session.settings.allowJavascript
            )
        )
    }
