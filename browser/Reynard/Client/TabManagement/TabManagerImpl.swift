...
final class TabManagerImplementation: NSObject, TabManager {
    ...
    private func persistState() {
        // Amnesic Mode: don't write the tab list to disk at all, matching
        // isPrivateMode's "nothing touches disk" guarantee for everything
        // else (see SessionManager.createSession).
        guard !Prefs.AmnesicPreferences.enabled else { return }
...
extension TabManagerImplementation: NavigationDelegate {
    ...
    func onLoadRequest(session: GeckoSession, request: LoadRequest) async -> AllowOrDeny {
        if let decision = await evaluateAntiXSS(for: request) {
            return decision
        }
    ...
    // Returns nil to mean "no opinion, fall through to normal handling";
    // returns an explicit decision when the anti-XSS filter has already
    // resolved this navigation (either the person confirmed it or the
    // check found nothing suspicious in the first place).
    private func evaluateAntiXSS(for request: LoadRequest) async -> AllowOrDeny? {
        guard Prefs.AntiXSSPreferences.enabled,
              AntiXSSFilter.shared.isSuspicious(request.uri) else {
            return nil
        }
        
        let proceeds = await AntiXSSFilter.shared.confirmProceed(for: request.uri)
        return proceeds ? nil : .deny
    }
